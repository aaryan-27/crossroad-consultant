...
export function Logo({ light = false, className }: { light?: boolean; className?: string }) {
  return (
    <Link href="/" className={cn("group inline-flex items-center gap-3", className)} aria-label="Agarwal Realtors — Home">
      <span className="relative grid h-11 w-11 place-items-center rounded-lg bg-gold-gradient shadow-gold transition-transform duration-300 group-hover:scale-105">
        <span className="font-display text-lg font-bold leading-none text-navy">AR</span>
      ...
      <span className="flex flex-col leading-none">
        ...
        >
          Agarwal <span className="text-gold-foil">Realtors</span>
        </span>
        <span
          className={cn(
