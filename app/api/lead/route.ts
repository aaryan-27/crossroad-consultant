...
async function notifyWhatsApp(lead: LeadPayload) {
  ...
  const body = [
    "🏠 *New Lead — Gorwal Properties & Developers*",
    `*Name:* ${lead.name}`,
    `*Phone:* ${lead.phone}`,
...
async function notifyEmail(lead: LeadPayload) {
  ...
  const html = `
    <h2 style="font-family:Georgia,serif;color:#0B1F3A">New Lead — Gorwal Properties & Developers</h2>
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: "Gorwal Properties & Developers <leads@gorwalproperties.in>",
      to: [toEmail],
