...
async function notifyWhatsApp(lead: LeadPayload) {
  ...
  const body = [
    "🏠 *New Lead — Evergreen Real Estate*",
    `*Name:* ${lead.name}`,
    `*Phone:* ${lead.phone}`,
...
async function notifyEmail(lead: LeadPayload) {
  ...
  const html = `
    <h2 style="font-family:Georgia,serif;color:#0B1F3A">New Lead — Evergreen Real Estate</h2>
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: "Evergreen Real Estate <leads@evergreenrealestate.in>",
      to: [toEmail],
